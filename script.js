...
function equalTo() {
    ...
    if (output.value.includes('Error')) {
        output.style.fontSize = "80px";
...
function showO(ok) {
    if (output.value.length < 18) {
        ...
        output.value += ok.innerHTML;
        output1.value += ok.value;
        if (output.value.includes('××') || output.value.includes('÷÷') || output.value.includes('%%') || output.value.includes('--') || output.value.includes('++') || output.value.includes('..')) {
            preview.innerHTML = "Error";
            return;
        }
        // alert(output1.value);
        let check = eval(output1.value);
        preview.innerHTML = (check == undefined)?"Error": check;
